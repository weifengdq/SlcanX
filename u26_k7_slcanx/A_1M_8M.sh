...
sudo ./slcandx/slcandx -t hw \
	-0c -0a${nominal[0]} -0A${data[0]} \
	-1c -1a${nominal[0]} -1A${data[0]} \
	-2c -2a${nominal[0]} -2A${data[0]} \
	-3c -3a${nominal[0]} -3A${data[0]} \
  $tty_device

for i in "${can_ifs[@]}"; do
	sudo ip link set can$i type can bitrate $nominal_bitrate dbitrate $data_bitrate fd on
